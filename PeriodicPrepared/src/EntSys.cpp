...
void EntSys::Norms1s2(vec &s1, mat &s2) {
	...
	double s2normSQ = abs(trace(ip * s2 * ip * s2)); // abs for safety
	double fac=sqrt(pow(s1norm,2)+s2normSQ);
	if(fac != 0.0){
	    s1=s1/fac;
	    s2=s2/fac;
	}
...
// EntSysCONT
mat EntSysCONT::Bmat(const mat &A,const vec &cx) {
    mat ret = sp * A * isp;
    mat C = isp*(pSys->f(cx));
    double kappa = 10.0;

    return ret+ret.t()-kappa*C*C.t();
}

// No longer in use, k0 is fixed
...
double EntSysCONT::abl(const vec &ox) {
	...
	//double eq = static_cast<Repressilator*>(pSys)->eq;
	double eq = 1.3794;
	if (eq - 0.5 < cx(0) && cx(0) < eq + 0.5) {
		//cout << "Removed 1" << endl;
		return -100000;
	}
	if (eq - 0.5 < cx(1) && cx(1) < eq + 0.5) {
		//cout << "Removed 2" << endl;
		return -100000;
	}
	if (eq - 0.5 < cx(2) && cx(2) < eq + 0.5) {
		//cout << "Removed 3" << endl;
		return -100000;
	}

	double cfac = dot(a, s1_vec(cx));
	vec lambda = eig_sym(Bmat(pSys->Amat(cx), cx));
	double max_lambda = -Infinity;
	for (int i = 0; i < dim; i++) {
		double lam = cfac + lambda(i);
		//cout << lam << endl;
		if (lam > max_lambda) {
			max_lambda = lam;
		}
	}
	//cout << max_lambda << endl;
	return max_lambda;
}
...
tuple<vec, mat, int, double> EntSysCONT::riem_subg(const vec &cx, bool OnlyEntEst) {
	...
	vec lambda;
	mat A = pSys->Amat(cx);
	if(!Bmat(A,cx).is_symmetric()){
	    cout<<"XXX"<<endl;
	    Bmat(A,cx).raw_print(cout);
	   // exit(1);
	}
	eig_sym(lambda, V, Bmat(A,cx));
